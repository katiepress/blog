...
![](/post/thetruth_part1_files/thetruth1_screenshot1.png){width=600px height=400px}

In this case it's pretty easy, the html nodes I'm interested in are simply "table" class. 

![](/post/thetruth_part1_files/thetruth1_screenshot2.png){width=600px height=400px}
...
I won't show all the tables just for the sake of space, but here is the first one. It looks like the "prod code" column is going to cause issues when I try and map them to one dataframe, because in some cases there are hyperlinks which results in inconsistent column names.
...
There is still some cleaning to be done. I have to fix the last episode of season 9, because it's actually two episodes (201/202). In the wikipedia table it's combined as one. 
...
my_theme <- theme(legend.position = "none",
        axis.text = element_text(family = "Agency FB", size = 12),
        axis.title.y = element_blank(),
        axis.title.x = element_text(family = "Agency FB", size = 14, vjust = -.5),
        panel.background = element_rect(color = "#AFAEB2", fill="#232928"),
        panel.grid.major = element_line(color = "#AFAEB2"))
```

Now I can plot the episodes by season and rating. The ggiraph package has a function that will add tooltips to a regular R plot, so I've added the episode number and title and it will pop up as your mouse hovers over each point. I put some additional theme formatting in my_theme that I'm not showing here due to space. But as you can see in the girafe object I decided to use the "Agency FB" font, it seemed appropriate. 
...
episode_desc <- table_df %>% 
  select(season, no_inseason, no_overall, description) %>% 
...
google_df <- googlesheets4::gs4_create("xfles_episodes",
                                   sheets = table_df)
```
